...
variable "service_account_id" {
  description = "ID of the service account for Instance Group"
  type        = string
  default     = "ajed6f7g849fg8ukd2a8"  
}
...
variable "lamp_image_id" {
  description = "ID of the LAMP image for Instance Group"
  type        = string
...
# ===== ТИП ДИСКА =====
variable "disk_type" {
  description = "Type of disk (network-ssd or network-hdd)"
  type        = string
  default     = "network-hdd"
}
# ===== КОНЕЦ ТИПА ДИСКА =====
...
variable "instance_group_resources" {
  description = "Resources for Instance Group VMs"
  type = object({
    cores         = number
    memory        = number
    core_fraction = number
...
variable "kms_key_name" {
  description = "Name of the KMS key for bucket encryption"
  type        = string
